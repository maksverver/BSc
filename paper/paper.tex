...
Suprisingly, the execution times for the different density values are comparable.
Figure \ref{fig-eratosthenes-bender_vss} shows that the lower density parameters do
indeed use a lot more memory. It appears that the advantage of low density is
...
that is available. Since execution times are similar, we select
$\delta=0.5$ for future experiments in order to minimize memory use.

% TODO: test with larger density!
...
\begin{figure}[p]
\centering
\includegraphics[width=83mm]{fig-eratosthenes_wctime}
...
\begin{figure}[p]
\centering
\includegraphics[width=83mm]{fig-eratosthenes_vss}
