...
address space has been allocated by a process; typically called the
\emph{virtual set size}.
Since a single page can be mapped into memory more than once (either in multiple
processes, or in the same process) this is not exactly equal to the amount of
memory allocated for the process, but since the majority of the memory used by
the data structures is mapped at exactly one location, it is good enough as
an indication of memory use.
...
program (called "wall clock time"). This has the disadvantage that concurrently
executing processes affect the runtime of the process being measured, so on a
busy system the reported runtime may vary with different executions.

A different way to measure time is using the statistics the kernel collects
of how many seconds the CPU spends executing instructions for the process
...
system spends waiting (for example, for a page to be swapped in from disk)
or the time spend by other processes on behalf of the executing process
...
Therefore, we decided to measure wall clock time. In order to deal with
variations across multiple executions, each experiment is run three times,
and the median value is used for further analysis.
...
running a second time while reading the stored values. As a result, the mock
set implementation has very little memory and runtime overhead.
...
\begin{tabular}{ l l l l }
\hline
\textbf{Case} & \textbf{Parameters} & \textbf{Iterations} & \textbf{Transitions} \\
\hline
...
and the properties of the execution of the model. Note that the number of transitions
relative to the number of iterations indicates the percentage of insertion operations
that results in a new element being added to the set.
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-queuesize}
\caption{Queue size}
\label{fig-quesize}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-transitions}
\caption{Transitions}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-eratosthenes-btree_wctime}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-eratosthenes-btree_vss}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-leader2_wctime}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-leader2_vss}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-peterson_wctime}
...
\begin{figure}
\centering
\includegraphics[width=83mm]{fig-peterson_vss}
...
% - relate to insert/test ratio
% - relate to hardware (not many levels of hierarchy)

\section{Conclusion}
The experimental results clearly show that in terms of both memory usage and execution time,
the cache-oblivious data structure proposed by Bender, Duan, Iacono and Wu is outperformed
by traditional data structures in our test scenario.
The advantage of more cache-friendly behaviour does not appear to be large enough to
compensate for the increased complexity of the data structure, which results in higher
memory requirements and increased computational overhead.
If the data structure does have asymptotic performance benefits, then realistic work loads
on current hardware systems are not enough to reveal this.
...
for this research are limited in scope: only a single cache-oblivious data
structure has been examined, in a single test scenario, on a single hardware
and software platform. It is possible that in a different setting,
cache-oblivious data structure do compare favorably to their traditional
counterparts.

We hope that future research will focus not only on the theoretical performance
of newly developed algorithms and data structure, but also on the performance
in practice. Theoretical results are invalueble, but to be of greatest
